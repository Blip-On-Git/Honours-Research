...
if [ $stage -le 7 ]; then
  ...
  echo "Stage 7: making utterance subsets..."
  utils/subset_data_dir.sh --shortest data/train 100000 data/train_100kshort
  utils/subset_data_dir.sh data/train 150000 data/train_150k
  utils/subset_data_dir.sh data/train 200000 data/train_200k
...
if [ $stage -le 8 ]; then
  ...
  steps/train_mono.sh --boost-silence 1.25 --nj $nj_train --cmd "$train_cmd" \
                      data/train_100kshort data/lang exp/mono
...
if [ $stage -le 9 ]; then
  ...
  steps/align_si.sh --boost-silence 1.25 --nj $nj_train --cmd "$train_cmd" \
                    data/train_150k data/lang exp/mono exp/mono_ali_150k

  echo "Stage 9: training a triphone model with MFCC + delta + delta-delta features..."
  
  # train a first delta + delta-delta triphone system on a subset of 5000 utterances
  steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" \
                        2000 10000 data/train_150k data/lang exp/mono_ali_150k exp/tri1
...
if [ $stage -le 10 ]; then
  ...
  steps/align_si.sh --nj $nj_train --cmd "$train_cmd" \
                    data/train_200k data/lang exp/tri1 exp/tri1_ali_200k
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
                          ...
                          data/train_200k data/lang exp/tri1_ali_200k exp/tri2b
...
if [ $stage -le 11 ]; then
  ...
  steps/align_si.sh  --nj $nj_train --cmd "$train_cmd" --use-graphs true \
                     data/train_200k data/lang exp/tri2b exp/tri2b_ali_200k

  echo "Stage 11: training a triphone model with SAT..."

  # Train tri3b, which is LDA+MLLT+SAT on 10k utts
  steps/train_sat.sh --cmd "$train_cmd" 2500 15000 \
                     data/train_200k data/lang exp/tri2b_ali_200k exp/tri3b
