...
for speaker in aux_corpus:
    for recording in speaker:
        ...
        id = recording.attrib['audio'].split('/')[-1].split('.')[0].replace("nchltAux1_zul","nchlt_zul")
        transcription = recording[0].text

        recording_name = recording.attrib['audio'].split('/')[-1].split('.')[0].replace("nchltAux1_zul","nchlt_zul")
        utt_id = recording_name
        spk_id = recording_name.split('_')[2]
