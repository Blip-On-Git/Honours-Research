...
aux_dict_name = sys.argv[3]
dictionary = open(dict_name+'.dict', 'r')
aux_dictionary = open(aux_dict_name+'.dict', 'r')
...
if lines[-1]=="":
    del lines[-1]    
print(f"original lexicon size: {len(lines)}")

raw_aux_lines = aux_dictionary.readlines()
# if the last line is empty, remove it
if raw_aux_lines[-1]=="":
    del raw_aux_lines[-1]
...
aux_vocab = set()
aux_lines = []
for aux_line in raw_aux_lines:
    
    if not (aux_line.split()[0] in aux_vocab):
        
        aux_vocab.add(aux_line.split()[0])

        if len(aux_line.split())>1:
            aux_lines.append(aux_line)


lines = lines+aux_lines
# if the last line is empty, remove it
if lines[-1]=="":
    del lines[-1]
print(f"lexicon size with aux: {len(lines)}")
...
for i in range(len(lines)):
    ...
    if lines[i]!="":
        ...
        line_space_delim = ' '.join(lines[i].split())
        lines[i] = line_space_delim

lines = list(dict.fromkeys(lines))
print(f"lexicon size with aux no duplicates: {len(lines)}")
