...
if [ $stage -le 7 ]; then
  ...
  utils/subset_data_dir.sh data/train 60000 data/train_60k
...
if [ $stage -le 9 ]; then
  ...
  steps/align_si.sh --boost-silence 1.25 --nj $nj_train --cmd "$train_cmd" \
                    data/train_30k data/lang exp/mono exp/mono_ali_30k

  echo "Stage 9: training a triphone model with MFCC + delta + delta-delta features..."
  
  # train a first delta + delta-delta triphone system on a subset of 5000 utterances
  steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" \
                        2000 10000 data/train_30k data/lang exp/mono_ali_30k exp/tri1
...
if [ $stage -le 10 ]; then
  ...
  steps/align_si.sh --nj $nj_train --cmd "$train_cmd" \
                    data/train_60k data/lang exp/tri1 exp/tri1_ali_60k
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
                          ...
                          data/train_60k data/lang exp/tri1_ali_60k exp/tri2b
...
if [ $stage -le 11 ]; then
  ...
  steps/align_si.sh  --nj $nj_train --cmd "$train_cmd" --use-graphs true \
                     data/train_60k data/lang exp/tri2b exp/tri2b_ali_60k

  echo "Stage 11: training a triphone model with SAT..."

  # Train tri3b, which is LDA+MLLT+SAT on 10k utts
  steps/train_sat.sh --cmd "$train_cmd" 2500 15000 \
                     data/train_60k data/lang exp/tri2b_ali_60k exp/tri3b
