...
if [ ! -f $extractor/final.ie ] && [ $ivector_dim -gt 0 ]; then
  ...
  if [ $stage -le 5 ]; then
    ...
    steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
      --num-processes $num_processes --ivector-dim $ivector_dim\
      --online-cmvn-iextractor $online_cmvn_iextractor \
      data/${train_set}_sp_hires_60k exp/nnet3${nnet3_affix}/diag_ubm $extractor || exit 1;
  fi
